...
export async function GET(request: NextRequest) {
  try {
    ...
    const headersWithDescriptions = Object.entries(securityHeaders).map(([key, value]) => ({
      name: key,
      value: value,
...
function getHeaderDescription(header: string): string {
  ...
  const descriptions: Record<string, string> = {
    'Strict-Transport-Security': 'Forces browsers to use HTTPS for the specified duration',
